...
class MPCForceSummary2:
    ...
    def __init__(self):
        try:
            ...
            self.theDlxFileName = "MPC Force Summary2.dlx"
...
##            SOLUTION = str(selectedsols[-1])
            ...
            for SOLUTION in selectedsols:
                if len(node_ids) >0:
                    ...
                    if SOLUTION in lsols:
                        ...
                        solookup(gen,resultManager,node_ids)#,line_num[0],line_nums2[0])#,output_lines,output_lines2)
                    else:
        ...
        with open(os.path.join(output_directory, "MPC_Force.csv"), "w") as data:

            combined_output_lines = [str(MPCforce)+str(MPCmoment) for MPCforce,MPCmoment in [list_item for list_item in list(zip(output_lines,output_lines2))]]
            ...
            self.theUI.NXMessageBox.Show("Block Styler", NXOpen.NXMessageBox.DialogType.Error, str("Exception Type: " + str(exception_type) + "\n" + "Exception Line: " + str(line_number)))

def solookup(gen,resultManager,node_ids):#:,line_num,line_num2):#,output_lines,output_lines2):
    theSession  = NXOpen.Session.GetSession()
    lw = theSession.ListingWindow
    lw.Open()
                    ...
                    try:
                        ...
                        for loadcase in loadcases:
                            ...
                            for iteration in iterations:
                                ...
                                try:
                                    MPCF = iteration.Find("ResultType[[Reaction Force MPC][Nodal]]")
                                    resultTypes.append(MPCF)
                                except:
                                    NXOpen.UI.GetUI().NXMessageBox.Show("Block Styler", NXOpen.NXMessageBox.DialogType.Error, str("MPC Forces not found"))
                                try:
                                    MPCM = iteration.Find("ResultType[[Reaction Moment MPC][Nodal]]")
                                    resultTypes.append(MPCM)
                                except:
                                    NXOpen.UI.GetUI().NXMessageBox.Show("Block Styler", NXOpen.NXMessageBox.DialogType.Error, str("MPC Moments not found"))
                                    ...
                                    for node_id in node_ids:
                                        if int(node_id) in lst_of_nodes:
                                            try:
                                                ...
                                                lw.WriteLine("  Node " + str(node_id) + " " + str(resultType.Name) + ":")
                                                ...
                                                #NXOpen.UI.GetUI().NXMessageBox.Show("Block Styler", NXOpen.NXMessageBox.DialogType.Error, str(traceback.print_exception()))
                                                exception_type, exception_object, exception_traceback = sys.exc_info()
                                                line_number = exception_traceback.tb_lineno
                                                NXOpen.UI.GetUI().NXMessageBox.Show("Block Styler", NXOpen.NXMessageBox.DialogType.Error, str("Exception Type: " + str(exception_type) + "\n" + "Exception Line: " + str(line_number)))
                                        else:
