...
class BeamForceSummary3:
    ...
    def __init__(self):
        try:
            ...
            self.theDlxFileName = "Beam Force Summary3.dlx"
...
def get_force(elem_ids,self,selectedsols) :
    header_line = "Element ID,Node ID,Axial Force NXX,Bending Moment MYY,Bending Moment MZZ,Torsional Moment MXX,Shear Force QXY,Shear Force QXZ,Solution,Subcase,Increment,Time,Result File Directory, Result File Name" + "\n"
...
##            SOLUTION = str(selectedsols[-1])
        ...
        with open(os.path.join(output_directory, "Beam_Force.csv"), "w") as data:
...
##                                            lst_of_nodes.append(solResult.AskNodeLabel(idx)) #for each node in result, add the node label (id) to the list of node labels
                                    ...
                                    if resultType.Quantity == NXOpen.CAE.Result.Quantity.BeamResultants and resultType.Location == NXOpen.CAE.Result.Location.ElementNodal:
                                        components = resultType.AskComponents()
                                        ...
                                        for element_id in elem_ids:
                                            if int(element_id) in lst_of_elems:
                                                try:
                                                    ...
                                                    lw.WriteLine("  Element " + str(element_id) + " Beam Force:")
                                                    for i in range(0,number_res_comps):                        
                                                        lw.WriteLine("Component = [" + str(components[0][i]) + "]   Value:   [" + str(elemValues[i]/1000) + "]")
                                                    outline = str(element_id) + "," + str(node_ids[0]) + "," +  ','.join([str(elem/1000) for elem in elemValues[:6]])+ "," + str(solution.Name) + "," + str(loadcase.Name) + "," + iteration_Name + "," + str(theResultFileName[0]) + "," + str(theResultFileName[1])
                                                    output_lines.append(outline)
                                                    outline = str(element_id) + "," + str(node_ids[-1]) + "," +  ','.join([str(elem/1000) for elem in elemValues[6:]])+ "," + str(solution.Name) + "," + str(loadcase.Name) + "," + iteration_Name + "," + str(theResultFileName[0]) + "," + str(theResultFileName[1])
                                                    output_lines.append(outline)
                                                    #line_num +=1
